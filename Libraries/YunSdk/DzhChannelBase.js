...
/**
 * Shared base for platform-specific DzhChannel implementations.
 */
class DzhChannelBase extends EventTarget {
  CONNECTING: number;
  OPEN: number;
  CLOSING: number;
  CLOSED: number;
  ...
  close(): void {
    if (this.readyState === DzhChannelBase.CLOSING ||
        this.readyState === DzhChannelBase.CLOSED) {
      return;
    }

    if (this.readyState === DzhChannelBase.CONNECTING) {
