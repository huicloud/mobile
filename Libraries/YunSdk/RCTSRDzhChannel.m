...
#import "com/dzhyun/sdk/Pb2Json.h"
#include "IOSClass.h"
...
static NSString *const RCTSRDzhChannelAppendToSecKeyString = @"258EAFA5-E914-47DA-95CA-C5AB0DC85B11";

static inline int32_t validate_dispatch_data_partial_string(NSData *data);
static inline void RCTSRFastLog(NSString *format, ...);

@interface NSData (RCTSRDzhChannel)
...
@interface NSString (RCTSRDzhChannel)
...
@interface NSURL (RCTSRDzhChannel)
...
@interface _RCTSRRunDzhLoopThread : NSThread
...
@implementation NSData (RCTSRDzhChannel)
...
@implementation NSString (RCTSRDzhChannel)
...
NSString *const RCTSRDzhChannelErrorDomain = @"RCTSRDzhChannelErrorDomain";
NSString *const RCTSRDzhHTTPResponseErrorKey = @"HTTPDzhResponseStatusCode";

// Returns number of bytes consumed. Returning 0 means you didn't match.
// Sends bytes to callback handler;
typedef size_t (^stream_scanner)(NSData *collected_data);

typedef void (^data_callback)(RCTSRDzhChannel *webSocket,  NSData *data);

@interface RCTSRDzhIOConsumer : NSObject
...
@interface RCTSRDzhIOConsumerPool : NSObject

- (instancetype)initWithBufferCapacity:(NSUInteger)poolSize NS_DESIGNATED_INITIALIZER;

- (RCTSRDzhIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
- (void)returnConsumer:(RCTSRDzhIOConsumer *)consumer;

@end

@interface RCTSRDzhChannel ()  <NSStreamDelegate>
...
@implementation RCTSRDzhChannel
{
  ...
  __strong RCTSRDzhChannel *_selfRetain;

  NSArray *_requestedProtocols;
  RCTSRDzhIOConsumerPool *_consumerPool;
}
...
- (void)_RCTSR_commonInit;
{
  ...
  _consumerPool = [RCTSRDzhIOConsumerPool new];
...
{
  assert(_url);
  RCTAssert(_readyState == RCTSR_CONNECTING, @"Cannot call -(void)open on RCTSRDzhChannel more than once");
...
- (BOOL)_checkHandshake:(CFHTTPMessageRef)httpMessage;
{
  ...
  NSString *concattedString = [_secKey stringByAppendingString:RCTSRDzhChannelAppendToSecKeyString];
...
- (void)_HTTPHeadersDidFinish;
{
  ...
  if (responseCode >= 400) {
    ...
    [self _failWithError:[NSError errorWithDomain:RCTSRDzhChannelErrorDomain code:2132 userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"received bad response code from server %ld", (long)responseCode], RCTSRDzhHTTPResponseErrorKey:@(responseCode)}]];
    return;
  }

  if(![self _checkHandshake:_receivedHTTPHeaders]) {
    [self _failWithError:[NSError errorWithDomain:RCTSRDzhChannelErrorDomain code:2133 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Invalid Sec-WebSocket-Accept response"]}]];
    return;
  }
  ...
  if (negotiatedProtocol) {
    ...
    if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
      [self _failWithError:[NSError errorWithDomain:RCTSRDzhChannelErrorDomain code:2133 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Server specified Sec-WebSocket-Protocol that wasn't requested"]}]];
      return;
    }
...
- (void)_readHTTPHeader;
{
  ...
  [self _readUntilHeaderCompleteWithCallback:^(RCTSRDzhChannel *socket,  NSData *data) {
...
- (void)didConnect
{
  ...
  if (_requestedProtocols) {
    CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Sec-DzhChannel-Protocol"), (__bridge CFStringRef)[_requestedProtocols componentsJoinedByString:@", "]);
  }
...
- (void)_handleFrameWithData:(NSData *)frameData opCode:(NSInteger)opcode;
{
  ...
  switch (opcode) {
    ...
    case RCTSROpCodeBinaryFrame:{
      IOSByteArray * bytes = [IOSByteArray arrayWithNSData:frameData];
      NSString *str = ComDzhyunSdkPb2Json_toJsonWithByteArray_(bytes);
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
  ...
  if (frame_header.payload_length == 0) {
    ...
    [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(RCTSRDzhChannel *socket, NSData *newData) {
      if (isControlFrame) {
...
- (void)_readFrameContinue;
{
  ...
  [self _addConsumerWithDataLength:2 callback:^(RCTSRDzhChannel *socket, NSData *data) {
    ...
    if (extra_bytes_needed == 0) {
      ...
      [socket _addConsumerWithDataLength:extra_bytes_needed callback:^(RCTSRDzhChannel *_socket, NSData *_data) {
...
- (void)_pumpWriting;
{
  ...
  if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
    ...
    if (bytesWritten == -1) {
      [self _failWithError:[NSError errorWithDomain:RCTSRDzhChannelErrorDomain code:2145 userInfo:@{NSLocalizedDescriptionKey: @"Error writing to stream"}]];
      return;
    }
...
- (BOOL)_innerPumpScanner
{
  ...
  RCTSRDzhIOConsumer *consumer = _consumers[0];
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode;
{
  if (_secure && !_pinnedCertFound && (eventCode == NSStreamEventHasBytesAvailable || eventCode == NSStreamEventHasSpaceAvailable)) {
    ...
    if (sslCerts) {
      ...
      if (!_pinnedCertFound) {
        dispatch_async(_workQueue, ^{
          [self _failWithError:[NSError errorWithDomain:RCTSRDzhChannelErrorDomain code:23556 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Invalid server cert"]}]];
        });
        return;
      }
    }
...
@implementation RCTSRDzhIOConsumer
...
@implementation RCTSRDzhIOConsumerPool
{
...
- (RCTSRDzhIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
{
  RCTSRDzhIOConsumer *consumer = nil;
  if (_bufferedConsumers.count) {
    consumer = _bufferedConsumers.lastObject;
    [_bufferedConsumers removeLastObject];
  } else {
    consumer = [RCTSRDzhIOConsumer new];
  }
...
@implementation NSURL (RCTSRDzhChannel)
...
static _RCTSRRunDzhLoopThread *networkThread = nil;
static NSRunLoop *networkRunLoop = nil;

@implementation NSRunLoop (RCTSRDzhChannel)

+ (NSRunLoop *)RCTSR_networkRunLoop
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    networkThread = [_RCTSRRunDzhLoopThread new];
...
@implementation _RCTSRRunDzhLoopThread
{
