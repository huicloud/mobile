...
#import "RCTDzhChannelExecutor.h"
...
@interface RCTDzhChannelExecutor () <RCTSRDzhChannelDelegate>

@end

@implementation RCTDzhChannelExecutor
{
  RCTSRDzhChannel *_socket;
...
- (void)setUp
{
  _jsQueue = dispatch_queue_create("com.facebook.React.DzhChannelExecutor", DISPATCH_QUEUE_SERIAL);
  _socket = [[RCTSRDzhChannel alloc] initWithURL:_url];
  ...
  if (![self connectToProxy]) {
    RCTLogError(@"Connection to %@ timed out. Are you running node proxy? If \
                ...
                address in `RCTDzhChannelExecutor.m`.", _url);
    [self invalidate];
    return;
  }
...
{
  RCTLogError(@"DzhChannel connection failed with error %@", error);
}
